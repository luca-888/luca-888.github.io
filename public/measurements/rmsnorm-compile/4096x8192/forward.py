...
# kernel path: /tmp/torchinductor_root/x5/cx5pban4qxf7lc65ie7bjjfl5c64ticixgr2lzynrh22ht3eoqui.py
...
#   %rsqrt : Tensor "f32[4096, 1][1, 1]cuda:0" = PlaceHolder[target=rsqrt]
#   %arg1_1 : Tensor "bf16[8192][1]cuda:0" = PlaceHolder[target=arg1_1]
#   %convert_element_type : Tensor "f32[4096, 8192][8192, 1]cuda:0"[num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%arg0_1, torch.float32), kwargs = {})
#   %pow_1 : Tensor "f32[4096, 8192][8192, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.pow.Tensor_Scalar](args = (%convert_element_type, 2), kwargs = {})
...
#   %rsqrt : Tensor "f32[4096, 1][1, 1]cuda:0"[num_users=2] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add,), kwargs = {})
#   %mul : Tensor "f32[4096, 8192][8192, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type, %rsqrt), kwargs = {})
...
#   %mul_1 : Tensor "f32[4096, 8192][8192, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%mul, %convert_element_type_1), kwargs = {})
#   %convert_element_type_2 : Tensor "bf16[4096, 8192][8192, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_1, torch.bfloat16), kwargs = {})
...
def triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_0(in_out_ptr0, in_ptr0, in_ptr1, out_ptr0, xnumel, r0_numel, XBLOCK : tl.constexpr, R0_BLOCK : tl.constexpr):
    ...
    for r0_offset in range(0, r0_numel, R0_BLOCK):
        ...
        rindex = r0_index
        r0_1 = r0_index
        tmp11 = tl.load(in_ptr0 + (r0_1 + 8192*x0), r0_mask, eviction_policy='evict_first', other=0.0).to(tl.float32)
        ...
        tl.store(out_ptr0 + (r0_1 + 8192*x0), tmp17, r0_mask)
''', device_str='cuda')
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(0):
            ...
            buf0 = empty_strided_cuda((4096, 1), (1, 4096), torch.float32)
            buf1 = reinterpret_tensor(buf0, (4096, 1), (1, 1), 0); del buf0  # reuse
            buf2 = empty_strided_cuda((4096, 8192), (8192, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [x, square, s, add, r, x_hat, gamma, y, to], Original ATen: [aten._to_copy, aten.pow, aten.mean, aten.add, aten.rsqrt, aten.mul]
            stream0 = get_raw_stream(0)
            triton_red_fused__to_copy_add_mean_mul_pow_rsqrt_0.run(buf1, arg0_1, arg1_1, buf2, 4096, 8192, stream=stream0)
            del arg0_1
            del arg1_1
        return (buf2, buf1, )
